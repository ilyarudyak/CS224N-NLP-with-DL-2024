...
"""
CS224N Spring 2024: Assignment 3 (v2)
run.py: Run Script for High-Performance NMT Model with PyTorch DataLoader
...
Options:
    ...
    --src-sp-model=<file>                   source sentencepiece model [default: src.model]
    --tgt-sp-model=<file>                   target sentencepiece model [default: tgt.model]
...
def train(args: Dict):
    ...
    """
    # Step DATA-1: Load vocab from file
    vocab = Vocab.load(args['--vocab'])
    pad_id = vocab.src['<pad>']

    # Step DATA-2: Load training and development datasets
    max_samples = int(args['--max-samples']) if args['--max-samples'] else None

    print('loading training dataset...', file=sys.stderr)
    train_dataset = TranslationDataset(
        src_file_path=args['--train-src'],
        tgt_file_path=args['--train-tgt'],
        src_vocab=vocab.src,
        tgt_vocab=vocab.tgt,
        src_sp_model=args['--src-sp-model'],
        tgt_sp_model=args['--tgt-sp-model'],
        max_samples=max_samples,
    )

    print('loading dev dataset...', file=sys.stderr)
    dev_dataset = TranslationDataset(
        src_file_path=args['--dev-src'],
        tgt_file_path=args['--dev-tgt'],
        src_vocab=vocab.src,
        tgt_vocab=vocab.tgt,
        src_sp_model=args['--src-sp-model'],
        tgt_sp_model=args['--tgt-sp-model'],
    ...
    # For diagnostic logging during validation
    dev_data_src_raw = read_corpus(args['--dev-src'], source='src', vocab_size=3000)
    dev_data_tgt_raw = read_corpus(args['--dev-tgt'], source='tgt', vocab_size=2000)
    example_sentence_src = dev_data_src_raw[3]
    example_sentence_tgt = dev_data_tgt_raw[3]
    ...
    pin_memory = bool(args['--pin-memory'])

    # Step DATA-3: Create DataLoaders
    ...
    # Step TRAIN-2: Initialize the NMT model
    model = NMT(
    ...
    while True:
        ...
        for src_padded, src_lengths, tgt_padded in train_loader:
            train_iter += 1

            optimizer.zero_grad()

            batch_size = src_padded.size(1)
            ...
            tgt_words_num_to_predict = (tgt_padded[1:] != pad_id).sum().item()
            ...
            if train_iter % valid_niter == 0:
                ...
                writer.add_text(
                    'example_translation_with_beam_search',
        ...
        # Check if maximum number of epochs has been reached (at epoch completion)
        if epoch >= int(args['--max-epoch']):
            print('reached maximum number of epochs (%d)!' % epoch, file=sys.stderr)
            exit(0)
