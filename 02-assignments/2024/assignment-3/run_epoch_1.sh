...
if [ "$1" = "train" ]; then
	CUDA_VISIBLE_DEVICES=0 python run.py train --max-epoch=1 --train-src=./zh_en_data/train.zh --train-tgt=./zh_en_data/train.en --dev-src=./zh_en_data/dev.zh --dev-tgt=./zh_en_data/dev.en --vocab=vocab.json --cuda --lr=5e-4 --patience=1 --valid-niter=200 --batch-size=32 --embed-size=1024 --hidden-size=768 --dropout=.3 
elif [ "$1" = "test" ]; then
